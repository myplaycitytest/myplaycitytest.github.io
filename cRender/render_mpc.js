...
{
	return Utils.getScaleScreenResolution(2, landscape);
};
...
Utils.createLayout = function(container, resolution, debug, ignoreCanvas)
{
	...
	cns.id = Utils.DOMScreenBackgroundId;
	cns.width = resolution.width;
	cns.height = resolution.height;
};
 ...
 * @param {DisplayObjectContainer} obj2 . Если этот параметр не передан, он равен this
 */
 ...
 * @param {Boolean} skipChildren нужно ли пропустить рекурсивную установку свойства у дочерних элементов
...
Sprite.prototype.renderBitmap = function(cns, x, y, w, h)
{
	...
	if(this.bitmap)
	{
		...
		if(fx < iw && fy < ih)
		{
			...
			{
			    cns.ctx.drawImage(this.cacheBitmap ? BitmapsCache.cache(this.bitmap) : this.bitmap, fx, fy, fw, fh, x, y, w, h);
            }
...
Sprite.prototype.render = function(cns, drawStatic, delta, drawMask)
{
	...
	if(!!this.static == !!drawStatic)
	{
		...
		var	ow = this.width * Utils.globalScale, 
			...
			// lef-top corner
			ox = center.x * Utils.globalScale - ow / 2, 
			oy = center.y * Utils.globalScale - oh / 2,
			// rotation
		...
		if(canvasMod)
		{
			...
			cns.ctx.translate(ox + ow / 2, oy + oh / 2);
			cns.ctx.rotate(or);
...
Stage.prototype.setBackgroundCanvas = function(canvas)
{
	if(canvas)
	{
		...
		if(this.backgroundCanvas) this.backgroundCanvas.ctx = this.backgroundCanvas.getContext('2d');
	}
};
...
Stage.prototype.tick = function()
{
    ...
    else
    {
		...
		this.render(this.canvas, true);
    	this.render(this.canvas, false, true, delta);
